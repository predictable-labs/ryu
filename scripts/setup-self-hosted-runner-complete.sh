...
#
# Usage:
#   ./setup-self-hosted-runner-complete.sh [OPTIONS]
#
# Options:
...
if [ "$NON_INTERACTIVE" = true ]; then
    echo "Running in NON-INTERACTIVE mode - all prompts will be auto-accepted"
fi
if [ "$FORCE_REINSTALL" = true ]; then
    echo "FORCE REINSTALL mode - all components will be reinstalled"
fi
echo ""
...
echo ""
echo "================================================================================"
echo "Step 5: Install Library Dependencies (including neo4j client dependencies)"
echo "================================================================================"
echo "Installing: libtool, libltdl-dev, libedit-dev, libssl-dev..."
echo "Note: These dependencies are required for building neo4j-client from source."
sudo apt-get install -y \
    libtool \
    libltdl-dev \
    libedit-dev \
...
echo ""
echo "================================================================================"
echo "Step 10: Install Rust (if not already installed)"
echo "================================================================================"
...
echo "================================================================================"
if [ -f /usr/local/lib/libduckdb.so ] && [ "$FORCE_REINSTALL" = false ]; then
    echo "DuckDB is already installed at /usr/local/lib/libduckdb.so"
    if [ "$NON_INTERACTIVE" = false ]; then
        read -p "Do you want to reinstall DuckDB? (y/N): " -n 1 -r
        echo
        REINSTALL_DUCKDB=$REPLY
    else
        echo "Non-interactive mode: skipping DuckDB reinstallation"
        REINSTALL_DUCKDB="n"
    fi

    if [[ $REINSTALL_DUCKDB =~ ^[Yy]$ ]]; then
...
echo "  3. Reload your shell to update environment variables:"
echo "     source ~/.bashrc"
