 ...
 *
 * lib/src/neo4j-client.h.  Generated from neo4j-client.h.in by configure.
 *
...
/**
 * An explicit transaction with a neo4j server.
 */
typedef struct neo4j_transaction neo4j_transaction_t;
 ...
 */
#define NEO4J_VERSION "5.0.7"
#define NEO4J_MAJOR_VERSION 5
#define NEO4J_MINOR_VERSION 0
#define NEO4J_PATCH_VERSION 7
#define NEO4J_DEVELOPMENT_VERSION ""

#define NEO4J_VERSION_NUMBER \
    ((NEO4J_MAJOR_VERSION << 20) | \
    (NEO4J_MINOR_VERSION << 12) | \
    (NEO4J_PATCH_VERSION << 4) | 0)
...
/**
 * Struct for representing Neo4j version supported by (this) client
...
typedef struct version_spec version_spec_t;
...
#define NEO4J_TRANSACTION_FAILED -40
#define NEO4J_TRANSACTION_DEFUNCT -41
#define NEO4J_FEATURE_UNAVAILABLE -42
...
extern const neo4j_type_t NEO4J_ELEMENTID;
extern const neo4j_type_t NEO4J_STRUCT;
extern const neo4j_type_t NEO4J_DATE;
extern const neo4j_type_t NEO4J_TIME;
extern const neo4j_type_t NEO4J_LOCALTIME;
extern const neo4j_type_t NEO4J_DATETIME;
extern const neo4j_type_t NEO4J_LOCALDATETIME;
extern const neo4j_type_t NEO4J_DURATION;
extern const neo4j_type_t NEO4J_POINT2D;
extern const neo4j_type_t NEO4J_POINT3D;
 ...
 *
 * The return value depends on the length of the string, which can be obtained,
 * in bytes, using neo4j_string_length(). For strings with a non-zero length,
 ...
 *         or `NULL` if the value is not a non-empty string.
 */
__neo4j_const
 ...
 *
 * @param [value] The neo4j node.
 * @return A neo4j value encoding the element ID of the node.
 */
__neo4j_pure
 ...
 *
 * @param [value] The neo4j relationship.
 * @return A neo4j value encoding the Element ID of the relationship.
 */
__neo4j_pure
 ...
 *
 * @param [value] The neo4j relationship.
 * @return A neo4j value encoding the Element ID of the start node.
 */
__neo4j_pure
 ...
 *
 * @param [value] The neo4j relationship.
 * @return A neo4j value encoding the Element ID of the end node.
 */
__neo4j_pure
...
/**
 * Set the extra dictionary (= neo4j_map) to be sent to server
 * with run messages (Bolt protocol 3+)
 *
 * @attention Once set, this map will be sent on every subsequent run.
 *            To cancel directives, do neo4j_set_extra(neo4j_null)
 *
 * @param [map] A neo4j_map containing directives to the server
 */
__neo4j_must_check
 ...
 * @attention This function is available when server protocol is Bolt 3 or higher.
 *
 * @param [connection] The connection.
 * @param [tx_timeout] Transaction timeout in milliseconds. If -1, default to server-side setting (dbms.transaction.timeout)
 * @param [tx_mode] Transaction mode string: "r" - read, "w" - write. If NULL, defaults to "w".
 * @param [dbname] Name of database in which to exec txn (no effect unless Neo4j 4+)
 * @return A `neo4j_transaction_t` tx. If tx->failed is set, the call failed; check tx->failure_code.
 *
 */
__neo4j_must_check
 ...
 * @attention This function is available when server protocol is Bolt 3 or higher.
 *
 * @param [transaction] The transaction (obtained from `neo4j_begin_tx`)
 ...
 * @return 1 if transaction is open; 0 if closed.
 */
 ...
 *
 * This flag is set when the server fails a transaction operation.
 * Examine the failure information with neo4j_tx_failure_code()
 * and neo4j_tx_failure_message().
 *
 * @param [tx] The transaction.
 * @return A non-zero value (errno) if the transaction has failed; 0 if not.
 */
__neo4j_must_check
 ...
 *
 * @param [tx] The transaction.
 * @return const char* database name
 */
__neo4j_must_check
 ...
 *
 * @param [tx] The transaction.
 * @return string (the bookmark token)
 */
__neo4j_must_check
 ...
 *
 * Use to clean up after transaction has failed, been committed, or
 * been rolled back.
 *
 * @param [tx] The transaction.
 * @return none
...
/**
 * Set supported versions communicated in the initial client-server
 * handshake.
 *
 * @param [config] The neo4j client configuration to update. 
 * @param [version_string] The version string, e.g., "5,4.4-4.2,3,2".
 **/
